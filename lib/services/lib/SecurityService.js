...
const { set, omitBy, isUndefined } = require('lodash')
...
const {
  exchange: {
    confirmTemplate: smsExchangeConfirmTemplate
  }
} = requireRoot('sms')
...
const SecurityAgent = keystone.list('SecurityAgent')
...
class SecurityService {
  ...
  async selectAddresses (user) {
    ...
    return addresses.reduce((res, addr) => {
      res[addr.type] = addr.value
      return res
    }, {})
  }

  async upsertAddresses ({user, addresses}) {
    await Promise.all(
      addresses.map(s => this.upsertAddress({ user, type: s.type, value: s.value }))
    )
  }
  ...
  async validatePhone (user) {
    ...
    smsService.send({
      to: request.level2.phone,
      msg: smsExchangeConfirmTemplate({ check: checkPhone.check })
    })

    log.debug(`[confirm-phone] user: ${user._id}, check: ${checkPhone.check}`)
  }
  ...
  async validateEmail (user) {
    ...
    log.debug(`[confirm-email] user: ${user._id}, check: ${checkEmail.check}`)
  ...
  async verify2FACode (user, requestModel) {
    assert(requestModel instanceof Verify2FACodeModel)
    const agent = await SecurityAgent.model
      .findOne({
        user: user._id,
    ...
    const isValid = !!verifyToken(agent.secret, requestModel.code)
    return isValid
  }
  ...
  async confirmLevel2Request (user, requestModel) {
    ...
    log.debug('requestModel.emailCode', requestModel.emailCode)
  ...
  async selectAgents (user, { principal, purpose }) {
    const agents = await SecurityUser.model
      .find(
        omitBy({
          user: user._id,
          principal,
    ...
    return agents.map(AgentModel.fromMongo)
  }

  async createAgent (user, createModel) {
    assert(createModel instanceof CreateAgentModel)
    const agent = await SecurityAgent.model.create({
      user: user._id,
      secret: createModel.secret,
      contract: createModel.contract,
      principal: createModel.principal,
      purpose: createModel.purpose
    })
    return AgentModel.fromMongo(agent)
  }

  async selectWorkerProfileServiceCategories () {
    return ServiceCategory.model.find()
  }

  async selectWorkerProfileCurrencies () {
    return Currency.model.find()
  }

  async selectClientProfileByAddress (address) {
    const signature = await SecuritySignature.model
      .findOne({
        address: address,
        type: 'ethereum-public-key'
    ...
    return ProfileClientModel.fromMongo({ client })
  }

  async selectWorkerProfileByAddress (address) {
    const signature = await SecuritySignature.model
      .findOne({
        address: address,
        type: 'ethereum-public-key'
    ...
    return ProfileWorkerModel.fromMongo({ worker })
  }
}
