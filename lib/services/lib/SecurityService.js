...
const {
  ...
  VerificationRequestReqruiterModel,
...
class SecurityService {
  ...
  async selectReqruiterProfile (user) {
    ...
    return ProfileReqruiterModel.fromMongo({ reqruiter, request })
  }
  ...
  async upsertReqruiterRequest (user, requestModel) {
    assert(requestModel instanceof VerificationRequestReqruiterModel)
    let request = await VerificationReqruiterRequest.model
      .findOne({
        user: user._id,
        status: 'created'
      })
    ...
    if (request) {
      ...
      request = await VerificationReqruiterRequest.model
        .create({
          user: user._id,
          verifiable,
          custom
    ...
    await Promise.all([
      ...(socials || []).map(s => ReqruiterSocial.model.create({
        ...s,
        request: request._id
      })),
      ...(services || []).map(async s => ReqruiterService.model.create({
        ...s,
        request: request._id
      }))
    ])
