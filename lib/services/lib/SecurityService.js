...
class SecurityService {
  ...
  async selectProfile (user) {
    const profile = await SecurityUser.model
      ...
      .populate({
        path: 'clientRequests',
        populate: [
          { path: 'verifiable.pageBackground' },
          { path: 'verifiable.attachments' },
          { path: 'regular.specializations' }
        ]
      })
      .populate({
        path: 'workerRequests',
        populate: [
          { path: 'verifiable.pageBackground' },
          { path: 'verifiable.attachments' },
          { path: 'regular.currencies' }
        ]
      }).populate({
        path: 'recruiterRequests',
        populate: [
          { path: 'verifiable.pageBackground' },
          { path: 'verifiable.attachments' }
        ]
  ...
  async selectWorkerProfile (user) {
    ...
    return ProfileWorkerModel.fromMongo({ worker, request })
  }
  ...
  async selectRecruiterProfile (user) {
    ...
    return ProfileRecruiterModel.fromMongo({ recruiter, request })
  }
  ...
  async requireSignature ({ type, value, roles }) {
    const signature = await SecuritySignature.model
      .findOne({ value, type })
      .populate('user')
    ...
    if (signature) {
      if (roles) {
        signature.user.worker.isRequested = roles.isWorker
        signature.user.client.isRequested = roles.isClient
        signature.user.recruiter.isRequested = roles.isRecruiter
        await signature.user.save()
      }
      return signature
    }
    ...
    const u = await SecurityUser.model
      .create({
        name: `${type}:${value}`,
        worker: { isRequested: roles.isWorker },
        client: { isRequested: roles.isClient },
        recruiter: { isRequested: roles.isRecruiter }
      })
