...
const { ImageModel, AttachmentModel, ServiceCategoryModel, CurrencyModel } = requireRoot('lib/models/Core')
...
const schemaFactory = () => ({
  ...
  client: Joi.object().keys({
    ...
    specializations: Joi.array().items(Joi.object().type(ServiceCategoryModel)).allow(null)
  }),
...
module.exports.model = class ProfileModel extends AbstractModel {
  ...
  static fromMongo (data, context = {}, options) {
    ...
    const { level1, level2, level3, level4, client, worker, recruiter, requests, clientRequests, workerRequests, recruiterRequests } = data

    const request1 = requests.find(r => r.level === 'level-1' && r.status === 'created')
    const request2 = requests.find(r => r.level === 'level-2' && r.status === 'created')
    const request3 = requests.find(r => r.level === 'level-3' && r.status === 'created')
    const request4 = requests.find(r => r.level === 'level-4' && r.status === 'created')
    const clientRequest = clientRequests.find(r => r.status === 'created')
    const workerRequest = workerRequests.find(r => r.status === 'created')
    const recruiterRequest = recruiterRequests.find(r => r.status === 'created')
    ...
    return new ProfileModel({
      ...
      client: {
        submitted: !clientRequest ? null : {
          ...clientFragment(clientRequest.verifiable),
          validationComment: clientRequest.validationComment
        },
        approved: (!clientRequest || !clientRequest.isValid) ? null : clientFragment(clientRequest.verifiable),
        isRequested: client.isRequested,
        specializations: client.specializations
      },
      worker: {
        submitted: !workerRequest ? null : {
          ...workerFragment(workerRequest.verifiable),
          validationComment: workerRequest.validationComment
        },
        approved: (!workerRequest || !workerRequest.isValid) ? null : workerFragment(workerRequest.verifiable),
        isRequested: worker.isRequested,
        currencies: worker.currencies,
        hourlyCharge: worker.hourlyCharge
      },
      recruiter: {
        submitted: !recruiterRequest ? null : {
          ...recruiterFragment(recruiterRequest.verifiable),
          validationComment: recruiterRequest.validationComment
        },
        approved: (!recruiterRequest || !recruiterRequest.isValid) ? null : recruiterFragment(recruiterRequest.verifiable),
        isRequested: recruiter.isRequested
      },
...
function clientFragment (client) {
  return {
    name: client.name,
    type: client.type,
    intro: client.intro,
    pageBackground: ImageModel.fromMongo(client.pageBackground),
    website: client.website,
    email: client.email,
    attachments: !client.attachments
      ? null
      : client.attachments.map(a => AttachmentModel.fromMongo(a))
  }
}
...
function recruiterFragment (recruiter) {
  return {
    intro: recruiter.intro,
    pageBackground: ImageModel.fromMongo(recruiter.pageBackground),
    attachments: !recruiter.attachments
      ? null
      : recruiter.attachments.map(a => AttachmentModel.fromMongo(a))
  }
}
