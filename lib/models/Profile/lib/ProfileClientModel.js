...
module.exports.model = class ProfileClientModel extends AbstractModel {
  ...
  static fromMongo ({ client, request, user }, context, options) {
    return new ProfileClientModel({
      ...
      isRequested: user && user.client && user.client.isRequested
    })
  }
}
...
function clientFragment (data) {
  return {
    ...
    regular: {
      specializations: !data.regular.specializations ? null : data.regular.specializations.map(ServiceCategoryModel.fromMongo)
    },
    custom: data.custom,
