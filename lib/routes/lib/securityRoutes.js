...
router.get('/me/full', authenticate(), asyncHandler(async (req, res) => {
  ...
  const reqruiter = await securityService.selectReqruiterProfile(user)
  res.send({
    worker,
    reqruiter,
    profile
...
router.post('/me/profile/reqruiter', authenticate(), asyncHandler(async (req, res) => {
  const { user } = req.securityContext
  await securityService.upsertReqruiterRequest(
    user,
    VerificationRequestReqruiterModel.fromJson(req.body)
  )
  const reqruiter = await securityService.selectReqruiterProfile(user)
  res.send(reqruiter)
}))
