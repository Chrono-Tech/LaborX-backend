...
router.get('/person', asyncHandler(async (req, res) => {
  const person = await securityService.selectPerson({ value : req.query.address })
  if (person) {
    res.send(person)
  } else {
    res.sendStatus(404)
...
router.get('/profile', asyncHandler(async (req, res) => {
  const person = await securityService.selectPerson({ address : req.query.address })
  const profile = await securityService.selectProfile({ _id: person.id })
  res.send(profile)
}))
