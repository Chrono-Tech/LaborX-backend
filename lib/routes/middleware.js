...
const { deepSortByKey, recoverPublicKey } = requireRoot('lib/utils')
...
function authenticate (
  ...
  return asyncHandler(async (req, res, next) => {
    ...
    if (type === AUTH_TYPE_SIGNATURE) {
      ...
      const publicKey = recoverPublicKey(data, value)
...
function errorHandler(err, req, res, next) {
  if (log.trace()) {
    log.trace({
      ...
      url: req.url,
      body: req.body
    }, 'errorHandler; error while handling request, error: %s', err)
  } else if (log.debug()) {
    log.debug({
      method: req.method,
      url: req.url
