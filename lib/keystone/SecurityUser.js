...
SecurityUser.add('Notifications', {
  notifications: {
    exchange: notificationsFragment(channel => ({
      userLoggedIn: `User logged in (${channel})`,
      profileUpdated: `Profile updated (${channel})`,
      transferReceived: `Transfer received (${channel})`,
      transferSent: `Transfer sent (${channel})`,
      limitOrderPublished: `Limit order published (${channel})`,
      limitOrderFilled: `Limit order filled (${channel})`,
      limitOrderExpired: `Limit order expired (${channel})`,
      marketOrderExecuted: `Market order executed (${channel})`
    })),
    laborx: notificationsFragment(channel => ({
      userLoggedIn: `User logged in (${channel})`,
      profileUpdated: `Profile updated (${channel})`
    }))
  }
