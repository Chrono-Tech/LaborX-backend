...
SecurityUser.add({
  ...
  roles: {
    isClient: { type: Boolean, label: 'Is Client', initial: true },
    isWorker: { type: Boolean, label: 'Is Worker', initial: true },
    isRecruiter: { type: Boolean, label: 'Is Recruiter', initial: true }
  }
})
...
SecurityUser.add('Residence', {
  ...
  worker: {
    attachments: { type: Types.Relationship, ref: 'WorkerProfile', label: 'Worker Profile', noedit: true, many: false },
    isValid: { type: Boolean, label: 'WP', hidden: true },
    isRequested: { type: Boolean, label: 'Is Requested' }
  },
  recruiter: {
    attachments: { type: Types.Relationship, ref: 'RecruiterProfile', label: 'Recruiter Profile', noedit: true, many: false },
    isValid: { type: Boolean, label: 'RP', hidden: true },
    isRequested: { type: Boolean, label: 'Is Requested' }
  },
  client: {
    attachments: { type: Types.Relationship, ref: 'ClientProfile', label: 'Client Profile', noedit: true, many: false },
    isValid: { type: Boolean, label: 'CP', hidden: true },
    isRequested: { type: Boolean, label: 'Is Requested' }
  }
})
...
SecurityUser.relationship({ path: 'workerRequests', ref: 'VerificationWorkerRequest', refPath: 'user' })

SecurityUser.schema.virtual('workerRequests', {
  ref: 'VerificationWorkerRequest',
  localField: '_id',
  foreignField: 'user',
  justOne: false
})

SecurityUser.relationship({ path: 'clientRequests', ref: 'VerificationClientRequest', refPath: 'user' })

SecurityUser.schema.virtual('clientRequests', {
  ref: 'VerificationClientRequest',
  localField: '_id',
  foreignField: 'user',
  justOne: false
})

SecurityUser.relationship({ path: 'recruiterRequests', ref: 'VerificationRecruiterRequest', refPath: 'user' })

SecurityUser.schema.virtual('recruiterRequests', {
  ref: 'VerificationRecruiterRequest',
  localField: '_id',
  foreignField: 'user',
  justOne: false
})
...
SecurityUser.relationship({ path: 'collaborators', ref: 'CollaboratorProfile', refPath: 'user' })
...
SecurityUser.defaultColumns = 'name|15%, avatar|10%, level1.userName|15%, level1.isValid|6%, level2.isValid|6%, level3.isValid|6%, level4.isValid|6%, worker.isValid|6%, recruiter.isValid|6%, client.isValid|6%, createdAt'
